...
const toLastNames = people => {
  return people.map(str => str.firstName + ' ' + str.lastName);
};
...
const addValues = (arr) => {
  return arr.reduce((accumulatedTotal, currentNumber) => {
    return currentNumber + accumulatedTotal;
  }, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((accumulatedTotal, currentPrice) => {
    return accumulatedTotal + currentPrice.purchasePrice;
  }, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce((acc, crr) => {
    // acc.push is pushing to the new empty array declared below
    acc.push(crr.name);
    return acc;
  }, []);
};
