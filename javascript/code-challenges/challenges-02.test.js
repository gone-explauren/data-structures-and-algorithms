...
const mapTwoToThe = (arr) => {
  // map() will not automatically return or create a new array if it is not written in one line as it was for previous challenges
  let newArr = [];
  arr.map(num => {
    newArr.push(Math.pow(2,num));
  });
...
const charCode = (arr) => {
  let newArr = arr.map(num => num.charCodeAt());
  return newArr;
};
